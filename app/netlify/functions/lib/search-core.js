...
// ⚠️ ABWEICHUNG von ../../../../code/search-core.js (sonst zeichengleich).
// Die Vorgängerfassung sprang zur ERSTEN Fundstelle des längsten Suchbegriffs.
// Das trifft bei kurzen Texten, versagt aber bei langen: Der längste Begriff
// ist meist der Produktname, und der steht im Titel. Das Fenster landete damit
// verlässlich am Textanfang — gleichgültig, wonach gefragt wurde.
//
// Gemessen (werkzeuge/antwort-eval.mjs): Bei „Wird ein im Vent-check offen
// gelassener Magnetkontakt wieder überwacht?" kommt „funk-magnetkontakt" im
// Artikel 9× vor. Gewählt wurde Position 201 — die Einleitung. Die Antwort
// stand bei Zeichen 10280 und wurde nie mitgeschickt. Vier der sechs
// Fehlschläge im Gold-Lauf gingen auf genau dieses Muster zurück.
//
// Stattdessen: das Fenster mit der höchsten DICHTE der Suchbegriffe. Wo sich
// „magnetkontakt", „vent-check" und „überwacht" häufen, steht die Antwort —
// nicht dort, wo das Produkt zum ersten Mal genannt wird.
//
// Zwei Feinheiten, beide aus Messungen entstanden:
//
//  1. Gezählt werden VORKOMMEN, nicht bloße Anwesenheit. Bei 6000 Zeichen
//     Fenster in einem 16.000-Zeichen-Artikel enthält fast jedes Fenster jeden
//     Begriff irgendwo — die Wertung sättigt, alle Fenster stehen gleich, und
//     der Gleichstand entscheidet dann wieder zugunsten des Textanfangs. Genau
//     das war bei „bei offener Fahrerhaustür scharf schalten?" zu sehen: acht
//     Fenster mit identischer Punktzahl.
//  2. Jeder Begriff wird durch seine Häufigkeit im Artikel geteilt. „scharf"
//     kommt im WiPro-Artikel 30× vor und taugt nicht zum Auffinden;
//     „fahrerhaustuer" kommt 4× vor und zeigt genau auf die Stelle. Seltene,
//     lange Begriffe wiegen deshalb schwerer.
//
// Offline gegen die Gold-Belege gemessen (24 mit wörtlich prüfbarer Fundstelle):
// vorher 14/24 im Kontext, danach 16/24.
//
// WICHTIG: Das ändert NICHTS am Ranking. `extractSnippet` wählt nur aus, welche
// Stelle eines bereits gefundenen Artikels gezeigt wird; die Trefferreihenfolge
// entsteht in searchWiki/searchSections und bleibt unberührt.
const MAX_FUNDSTELLEN = 40; // je Begriff — deckelt die Laufzeit bei Füllwörtern
const MAX_JE_FENSTER = 3;   // ein Begriff kann ein Fenster nicht allein tragen

export function extractSnippet(body, query, size = 800) {
  const text = String(body || '');
  if (text.length <= size) return text;
  const ntext = normalizeSearch(text);
  const terms = [...new Set(salientTerms(query))];

  // Alle Fundstellen sammeln (nach Position sortiert) und je Begriff zählen,
  // wie oft er im ganzen Artikel vorkommt.
  const fundstellen = [];
  const haeufigkeit = new Map();
  for (const term of terms) {
    let i = -1;
    let n = 0;
    while ((i = ntext.indexOf(term, i + 1)) >= 0) {
      if (n < MAX_FUNDSTELLEN) fundstellen.push({ pos: i, term });
      n += 1;
    }
    haeufigkeit.set(term, n);
  }
  if (!fundstellen.length) return text.slice(0, size);
  fundstellen.sort((a, b) => a.pos - b.pos);

  const gewicht = (term) => term.length / Math.max(1, haeufigkeit.get(term));

  // Jede Fundstelle als Fensteranker durchprobieren. Der Treffer sitzt bei
  // einem Viertel des Fensters: etwas Vorlauf, viel Umfeld danach.
  let besteStelle = fundstellen[0].pos;
  let bestePunkte = -1;
  for (const anker of fundstellen) {
    const von = Math.max(0, anker.pos - Math.floor(size / 4));
    const bis = von + size;
    const gezaehlt = new Map();
    let punkte = 0;
    for (const f of fundstellen) {
      if (f.pos >= bis) break;      // sortiert — ab hier liegt alles außerhalb
      if (f.pos < von) continue;
      const bisher = gezaehlt.get(f.term) || 0;
      if (bisher >= MAX_JE_FENSTER) continue;
      gezaehlt.set(f.term, bisher + 1);
      punkte += gewicht(f.term);
    }
    // Strikt größer: Bei Gleichstand gewinnt die frühere Stelle.
    if (punkte > bestePunkte) { bestePunkte = punkte; besteStelle = anker.pos; }
  }

  const hitAt = originalIndexForNormalized(text, besteStelle);
