...
Cette page décrit l'installation d'un WiPro III dans le VW T6 des années modèles 2015-2019. Le manuel d'installation commun de dix pages pour T6 et T6.1, statut `12/20`, est pertinent ; la révision générale du manuel d'installation WiPro III `1.8` ajoute la sécurité et le brochage du jeu de câbles à 20 broches.
...
|---|---|
| Profil du véhicule | exclusivement T6 2015-2019 avec `SW3 + SW4 + SW6` |
| Base d'appareils | Kit universel `100754`, WiPro à partir du numéro de série `0823-012` |
| Lignes CAN | T6 orange/marron et orange/vert ; l'écart T6.1 n'est pas transféré |
...
Les fichiers `VW.docx`, `Fahrzeugbesonderheiten.docx`, `WiPro 13safe.lock.docx` et `WiPro III12safe.lock.docx` précédemment référencés n'existent pas localement. Les déclarations qui en découlent – ​​telles que les prises CAN alternatives, les exclusions DoKa ou les cas particuliers safe.lock – ne seront donc pas conservées en tant que données d'installation approuvées.

1. Confirmez le modèle et l'année modèle à l'aide des documents du véhicule.
2. Différenciez clairement le T6 du T5 facelift et du T6.1.
3. Confirmez l'ensemble universel `100754` et le numéro de série de `0823-012`.
...
3. Pour le T6, réglez uniquement `SW3`, `SW4` et `SW6` sur `ON`.
4. Laissez tous les autres interrupteurs dans la position de base indiquée dans le manuel du véhicule.
5. N'utilisez pas le profil T6.1 `SW2 + SW3 + SW4 + SW6` illustré ci-dessous.
...
| orange/vert sur la boîte à fusibles/le tunnel de câbles | blanc/orange, broche `17` | CAN-High |
...
3. Mesurez l'orange/vert et connectez-vous à la broche blanche/orange `17` via un connecteur rempli de gel.
...
| WiPro ne répond pas à la télécommande du véhicule | Vérifiez les lignes CAN et le profil DIP ; Ne confondez pas les couleurs T6 avec T6.1 |
| aucune activité CAN en mode diagnostic | Vérifiez l'état du bus, connecteurs remplis de gel, broche orange/marron `18` et broche orange/verte `17` |
...
3. Documentez la position DIP `SW3 + SW4 + SW6`.
